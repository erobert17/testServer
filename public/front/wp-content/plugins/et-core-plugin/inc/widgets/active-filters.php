...
class ET_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display a list of active product filters.', 'xstore' );
		$this->widget_id          = 'et_layered_nav_filters';
		$this->widget_name        = '8theme - ' . esc_html__( 'Active Product Filters', 'xstore' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Active filters', 'xstore' ),
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) || ! empty( $filter_brand ) ) {
			...
			if ( ! empty( $_chosen_attributes ) ) {
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					foreach ( $data['terms'] as $term_slug ) {
						...
						echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'xstore' ) . '" href="' . esc_url( $link ) . '">' . esc_html( $term->name ) . '</a></li>';
					}
				}
			}
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'xstore' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( 'Min %s', 'xstore' ), wc_price( $min_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}
			if ( $max_price ) {
				$link = remove_query_arg( 'max_price', $base_link );
				/* translators: %s: maximum price */
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'xstore' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( 'Max %s', 'xstore' ), wc_price( $max_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}
			if ( ! empty( $rating_filter ) ) {
				foreach ( $rating_filter as $rating ) {
					...
					echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'xstore' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Rated %s out of 5', 'xstore' ), esc_html( $rating ) ) . '</a></li>';
				}
			}
			...
			if ( $filter_brand ) {
				...
				foreach ( $barnds as $key => $value ) {
					...
					/* translators: %s: maximum price */
					echo '<li class="chosen"><a class="remove-brand" rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'xstore' ) . '" href="' . esc_url( $link ) . '">' . $term->name . '</a></li>'; // WPCS: XSS ok.
				}
