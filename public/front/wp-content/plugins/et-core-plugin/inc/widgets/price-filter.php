...
class ET_Widget_Price_Filter extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = '8theme - ' . __( 'Filter Products by Price', 'xstore' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		$button_text = ($ajax_filters) ? esc_html__( 'Reset', 'xstore' ) : esc_html__( 'Filter', 'xstore' );

		echo '<form method="get" action="' . esc_url( $form_action ) . '">
			<div class="price_slider_wrapper">
				<div class="price_slider" style="display:none;"></div>
				<div class="price_slider_amount">
					<input type="text" id="min_price" name="min_price" value="' . esc_attr( $min_price ) . '" data-min="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_min_amount', $min ) ) . '" placeholder="' . esc_attr__( 'Min price', 'xstore' ) . '" />
					<input type="text" id="max_price" name="max_price" value="' . esc_attr( $max_price ) . '" data-max="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_max_amount', $max ) ) . '" placeholder="' . esc_attr__( 'Max price', 'xstore' ) . '" />
					<button type="submit" class="button et-reset-price ' . $class . '">' . $button_text . '</button>
					<div class="price_label" style="display:none;">
						' . esc_html__( 'Price:', 'xstore' ) . ' <span class="from"></span> &mdash; <span class="to"></span>
					</div>
