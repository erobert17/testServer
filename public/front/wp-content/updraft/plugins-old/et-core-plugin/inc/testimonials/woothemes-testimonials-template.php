...
function woothemes_testimonials ( $args = '' ) {
	...
	do_action( 'woothemes_testimonials_before', $args );
		...
		if ( ! is_wp_error( $query ) && is_array( $query ) && count( $query ) > 0 ) {
			...
			if ( $args['type'] == 'slider' ){
				$html .= '<div id="owl-testimonials" class="swiper-container '.$class.' testimonials" data-autoplay="'.(($args['interval']) ? $args['interval'] : "false").'"> '."\n";
            } elseif ( $args['type'] == 'slider-grid' ){
                $html .= '<div id="owl-testimonials" class="swiper-container '.$class.' testimonials" data-autoplay="'.(($args['interval']) ? $args['interval'] : "false").'" data-breakpoints="1" data-xs-slides="1" data-sm-slides="1" data-md-slides="1" data-lt-slides="2" data-slides-per-view="'.esc_attr($args['slider_columns']).'" data-slidesPerColumn="2"> '."\n";
			}
			...
			if($args['type'] == 'slider') {
				$tpl = '<div id="quote-%%ID%%" class="item swiper-slide %%CLASS%%"><div class="testimonial-slider-item"><blockquote class="testimonials-text">%%TEXT%%</blockquote><div class="testimonial-info"> %%AVATAR%% <div class="testimonial-author">%%AUTHOR%%</div></div> <div class="clear"></div></div></div>';
			} elseif($args['type'] == 'slider-grid') {
                $tpl = '<div id="quote-%%ID%%" class="item swiper-slide %%CLASS%%"><div class="testimonial-slider-item"><blockquote class="testimonials-text">%%TEXT%%</blockquote><div class="testimonial-info"> %%AVATAR%% <div class="testimonial-author">%%AUTHOR%%</div></div> <div class="clear"></div></div></div>';
			} elseif($args['type'] == 'grid') {
				$tpl = '<div id="quote-%%ID%%" class="item swiper-slide col-lg-4 %%CLASS%%"><blockquote class="testimonials-text">%%TEXT%%</blockquote><div class="testimonial-info">%%AVATAR%% <div class="testimonial-author">%%AUTHOR%%</div></div> <div class="clear"></div></div>';
			}
            ...
            if ( $args['navigation'] ) {
                $html .= '<div class="swiper-custom-left"></div>';
                $html .= '<div class="swiper-custom-right"></div>';
            }
