...
function etheme_child_styles() {
    ...
    if ( $is_redux && etheme_get_option('et_optimize_css') ) {
    ...
    if ( $is_redux && etheme_get_option('secondary_menu') ) {
        wp_enqueue_style("secondary-style",get_template_directory_uri().'/css/secondary-menu.css', array("parent-style"));
    }
...
if(!function_exists('etheme_add_body_classes')) {
    function etheme_add_body_classes($classes) {
        ...
        $classes[] = ( wp_is_mobile()) ? 'mobile-device' : ''; // new
...
if ( !function_exists('et_custom_styles') ) {
    function et_custom_styles () {
        ...
        // Top bar background color with opacity
        $top_bar_bg = etheme_get_option('top_bar_bg');
        $top_bar_opacity = etheme_get_option('top_bar_bg_opacity');
        $topBar_bg_color = (!empty($top_bar_bg['background-color']) && $top_bar_opacity < 1 && $top_bar_bg['background-color'] != 'transparent') ? $top_bar_bg['background-color'] : '';

        // Header background color with opacity
        $header_bg_color = etheme_get_option('header_bg');
        $header_bg_opacity = etheme_get_option('header_bg_opacity');
        $header_bg_color = (!empty($header_bg_color['background-color']) && $header_bg_opacity < 1 && $header_bg_color['background-color'] != 'transparent' ) ? et_hex_to_rgba($header_bg_color['background-color'],$header_bg_opacity) : '';

        // Fixed header background color with opacity 
        $fixed_header_bg_color = etheme_get_option('fixed_header_bg');
        $fixed_header_bg_opacity = etheme_get_option('fixed_header_bg_opacity');
        $fixed_header_bg_color = (!empty($fixed_header_bg_color['background-color']) && $fixed_header_bg_opacity < 1 && $fixed_header_bg_color['background-color'] != 'transparent' ) ? et_hex_to_rgba($fixed_header_bg_color['background-color'],$fixed_header_bg_opacity) : '';

        $header_padding = etheme_get_option('header_padding');
        $header_margin_bottom = etheme_get_option('header_margin_bottom');
        $header_margin_bottom = (empty($header_margin_bottom) ) ? 0 : $header_margin_bottom;
            ...
            $css .= '@media only screen and (min-width: 993px) {
                ...
                .header-smart-responsive .header-wrapper header > .container .container-wrapper {';
                    if ( !empty($header_padding['padding-top']) ) {
                        $css .= 'padding-top:' . $header_padding['padding-top'] . ';';
                    }
                    if ( !empty($header_padding['padding-right']) ) {
                        $css .= 'padding-right:' . $header_padding['padding-right'] . ';';
                    }
                    if ( !empty($header_padding['padding-bottom']) ) {
                        $css .= 'padding-bottom:' . $header_padding['padding-bottom'] . ';';
                    }
                    if ( !empty($header_padding['padding-left']) ) {
                        $css .= 'padding-left:' . $header_padding['padding-left'] . ';';
                    }
                $css .= '}
                .header-wrapper.header-xstore2 header .container-wrapper,
                .header-wrapper.header-center2 .container-wrapper {';
                    if ( !empty($header_padding['padding-top']) ) {
                        $css .= 'padding-top:' . $header_padding['padding-top'] . '!important;';
                    }
                    if ( !empty($header_padding['padding-right']) ) {
                        $css .= 'padding-right:' . $header_padding['padding-right'] . '!important;';
                    }
                    if ( !empty($header_padding['padding-bottom']) ) {
                        $css .= 'padding-bottom:' . $header_padding['padding-bottom'] . '!important;';
                    }
                    if ( !empty($header_padding['padding-left']) ) {
                        $css .= 'padding-left:' . $header_padding['padding-left'] . '!important;';
                    }
                $css .= '}
            }';
        }
        $css .= '.breadcrumbs-type-disable.et-header-not-overlap:not(.home) .header-wrapper {
        ...
        $et_selectors['active_stroke'] = '
            ...
            $css .= etheme_js2tring($et_selectors['active_color']) . '{ color: ' . $activeColor . '}';
            $css .= etheme_js2tring($et_selectors['active_bg']) . '{ background-color: ' . $activeColor . '}';
            $css .= etheme_js2tring($et_selectors['active_border']) . '{ border-color: ' . $activeColor . '}';
            $css .= etheme_js2tring($et_selectors['active_stroke']) . ' { stroke: ' . $activeColor . '}';
            ...
            $css .= '.onsale, .sale-value{';
            	$css .= (!empty($sale_color)) ? 'color:'.$sale_color.';' : '';
            	$css .= (!empty($sale_bg)) ? 'background-color:'.$sale_bg.';' : '';
            	$css .= (!empty($sale_br_radius)) ? 'border-radius:'.$sale_br_radius.'%;' : '';
            	$css .= (!empty($sale_width)) ? 'width:'.$sale_width.'em;' : '';
            	$css .= (!empty($sale_height)) ? 'height:'.$sale_height.'em; line-height:'. $sale_height . 'em;' : '';
            $css .= '}';

            $css .= '.et-header-full-width .main-header .container,
                .et-header-full-width .navigation-wrapper .container,
                .et-header-full-width .fixed-header .container {
                    max-width: ' . etheme_get_option('header_width') . 'px;
                }';

            $css .= '@media (min-width: 1200px){
                .container, div.container,
                .single-product .woocommerce-message, .single-product .woocommerce-error, .single-product .woocommerce-info {
                    width: ' . etheme_get_option('site_width') . 'px;
                }
                .footer:after {
                    width: ' . (etheme_get_option('site_width') - 30 ) . 'px;
                }
            ...
            if( ! empty( $header_bg_color)) {
                $css .= '.header-bg-block, .header-vertical {
                    background-color: ' .$header_bg_color . '!important;
                }';
            }

            if ( !empty( $fixed_header_bg_color )) {
            $css .= '.fixed-header {
                background-color: ' . $fixed_header_bg_color . '!important;
            }';
        }
        if ( ! empty( $topBar_bg_color)) {
            $css .= '.top-bar {
                background-color: '. et_hex_to_rgba($topBar_bg_color, $top_bar_opacity) . '!important;
            }';
        }

        $footer_padding = etheme_get_option('footer_padding');
        $f_t_p = isset($footer_padding['padding-bottom']) ? $footer_padding['padding-bottom'] : '';

        if ( !empty($f_t_p) ) {
            $css .= 'footer.footer:after {
                top: '. $f_t_p .';
            }';
        }

        // Secondary menu 
        $secondary_menu_border_width = etheme_get_option('secondary-menu-border-width');
        $secondary_menu_border_width_style = '';
        if ( $secondary_menu_border_width ) {
            if ( isset($secondary_menu_border_width['border-right']) && !empty($secondary_menu_border_width['border-right'])) {
                $secondary_menu_border_width_style .= 'border-right-width:' . $secondary_menu_border_width['border-right'] . ' !important;';
            }
            if ( isset($secondary_menu_border_width['border-bottom']) && !empty($secondary_menu_border_width['border-bottom'])) {
                $secondary_menu_border_width_style .= 'border-bottom-width:' . $secondary_menu_border_width['border-bottom'] . ' !important;';
            }
            if ( isset($secondary_menu_border_width['border-left']) && !empty($secondary_menu_border_width['border-left'])) {
                $secondary_menu_border_width_style .= 'border-left-width:' . $secondary_menu_border_width['border-left'] . ' !important;';
            }
            if ( isset($secondary_menu_border_width['border-top']) && !empty($secondary_menu_border_width['border-top'])) {
                $secondary_menu_border_width_style .= 'border-top-width:' . $secondary_menu_border_width['border-top'] . ' !important;';
            }
        }

        $secondary_menu_border_style = etheme_get_option('secondary-menu-border-style');
        $secondary_menu_border_style = (!empty($secondary_menu_border_style['border-style']) && $secondary_menu_border_style['border-style'] != 'solid') ? $secondary_menu_border_style['border-style'] : '';
        ...
        $secondary_links_border_width = etheme_get_option('secondary-links-border-width');

        $secondary_menu_border_color = etheme_get_option('secondary-menu-border-color');
        $secondary_menu_border_color_rgba = (is_array($secondary_menu_border_color) && isset($secondary_menu_border_color['rgba'])) ? $secondary_menu_border_color['rgba'] : '';

        $css .= '.secondary-menu-wrapper .menu {';
        $css .= 'border-color:'.$secondary_menu_border_color_rgba.' !important;';
        $css .= '}';

        if ( !empty($secondary_menu_border_width_style) && trim($secondary_menu_border_width_style) != '' ) {
            $css .= '.secondary-menu-wrapper .menu, .secondary-title{' . $secondary_menu_border_width_style . '}';
        }

        if ( trim($secondary_menu_border_style) != '' ) {
            $css .= '.secondary-menu-wrapper .menu {
                border-style:' . $secondary_menu_border_style .';
            }
            .secondary-title {
                border-left-style: ' . $secondary_menu_border_style . ';
                border-right-style: ' . $secondary_menu_border_style . ';
            }';
        }
        ...
        if (!empty($secondary_links_border_width) && trim($secondary_links_border_width) != '') {
            $css .= '.secondary-menu-wrapper .menu > li > a {
                border-bottom-width:' . $secondary_links_border_width . 'px;
            }';
        }
        ...
        $menu_links_border_width = etheme_get_option('menu-border-width');
        if ( $menu_links_border_width ) {
            $css .= '.menu-wrapper > .menu-main-container > .menu > li, .fullscreen-menu .menu > li {border-width: '.$menu_links_border_width['border-top'] . ' ' . $menu_links_border_width['border-right'] . ' ' . $menu_links_border_width['border-left'] . ' ' . $menu_links_border_width['border-bottom'].'}';
        }
        $menu_links_border_width_hover = etheme_get_option('menu-border-width-hover');
        if ( $menu_links_border_width_hover ) {
            $css .= '.menu-wrapper > .menu-main-container > .menu > li:hover, .fullscreen-menu .menu > li:hover {border-width: '.$menu_links_border_width_hover['border-top'] . ' ' . $menu_links_border_width_hover['border-right'] . ' ' . $menu_links_border_width_hover['border-left'] . ' ' . $menu_links_border_width_hover['border-bottom'].'}';
        }
        // Menu links 
        ...
        $menu_dr_border_width = etheme_get_option('menu_dropdown_border');
        if ( $menu_dr_border_width ) {
            $menu_dr_border_width = $menu_dr_border_width['border-top'] . ' ' . $menu_dr_border_width['border-right'] . ' ' . $menu_dr_border_width['border-bottom'] . ' ' . $menu_dr_border_width['border-left'];
        }
        ...
        if ( ! empty($light_buttons_radius) && trim($light_buttons_radius) != '' )  {
            $css .= 'input[type="submit"], .open-filters-btn .btn, .content-product .product-details .button, .woocommerce table.wishlist_table td.product-add-to-cart a, .woocommerce-Button, .et_load-posts .btn {
                border-radius:' . $light_buttons_radius . '!important;
            }';
        }
        if ( ! empty( $dark_buttons_radius ) && trim($dark_buttons_radius) != '' )  {
            $css .= '.btn-checkout, .et-wishlist-widget .wishlist-dropdown .buttons .btn-view-wishlist, .checkout-button, .single_add_to_cart_button.button, .before-checkout-form .button, 
            form.login .button, form.register .button, .empty-cart-block .btn, .form-submit input[type=submit] {
                border-radius: ' . $dark_buttons_radius .';
                }';
        }
        if ( ! empty($active_buttons_radius) && trim($active_buttons_radius) != '' )  {
            $css .= '.form-row.place-order .button {
                border-radius:' . $active_buttons_radius . ';
            }';
        }
        if ( ! empty( $menu_links_border_radius) && trim($menu_links_border_radius) != '' )  {
            $css .= '.menu-wrapper  > .menu-main-container > .menu > li, .fullscreen-menu .menu > li {
                border-radius:' . $menu_links_border_radius . ';
            }';
        }
        if ( !empty( $menu_links_border_style ) && trim($menu_links_border_style) != '' ) {
            $css .= '.menu-wrapper  > .menu-main-container > .menu > li, .fullscreen-menu .menu > li  {
                border-style:' . $menu_links_border_style . ';
            }';
        }
        if ( !empty( $menu_links_border_style_hover ) && trim($menu_links_border_style_hover) != '' ) {
            $css .= '.menu-wrapper  > .menu-main-container > .menu > li:hover, .fullscreen-menu .menu > li:hover {
                border-style:' . $menu_links_border_style_hover . ';
            }';
        }
        if ( !empty( $menu_dropdown_border_style) && trim($menu_dropdown_border_style) != '' )  {
            $css .= '.nav-sublist-dropdown  {
                border-style:' . $menu_dropdown_border_style . ';
            }';
        }

        if ( !empty($menu_dr_border_width) && trim($menu_dropdown_border_style) != '' ) {
            $css .= '.nav-sublist-dropdown {
                border-width:' . $menu_dr_border_width .';';
            $css .= '}';
        }

        if ( ! empty($f_menu_links_border_radius) && trim($f_menu_links_border_radius) != '' )  {
            $css .= '.fixed-header .menu-wrapper .menu > li {
            border-radius:' . $f_menu_links_border_radius . ';
            }';
        }
        if ( !empty($f_menu_links_border_style) && trim($f_menu_links_border_style) != '' ) { 
            $css .= '.fixed-header .menu-wrapper .menu > li {
            border-style:' . $f_menu_links_border_style . '!important;
            }';
        }
        if ( !empty($f_menu_links_border_style_hover) && trim($f_menu_links_border_style_hover) != '' ) { 
            $css .= '.fixed-header .menu-wrapper .menu > li:hover {
            border-style:' . $f_menu_links_border_style_hover . '!important;
            }';
        }
         
        $background_img = etheme_get_option('background_img');
        if(!empty($background_img['background-color'])) {
                $css .= '.bordered .body-border-left,
                .bordered .body-border-top,
                .bordered .body-border-right,
                .bordered .body-border-bottom {
                    background-color:' . $background_img['background-color'] . ';
            }';
        }
        ...
        $css .= '.header-logo img {
            max-width:' . esc_html( $logo_width ) . 'px;
        }';

    
        $tab_height = etheme_get_option('tab_height');
        
        if( etheme_get_option('tabs_scroll') ) {
            $css .= '.tabs-with-scroll.accordion .tab-content .tab-content-inner {
                max-height:' . esc_html( $tab_height ) .'px;
            }';
        }

        $s_ws_height = etheme_get_option('sidebar_widgets_height'); // sidebar widgets height
        
        if( etheme_get_option('sidebar_widgets_scroll') ) {
            $css .= '.archive.woocommerce-page.s_widgets-with-scroll .sidebar .sidebar-widget:not(.sidebar-slider):not(.etheme_widget_satick_block) > ul, .archive.woocommerce-page.s_widgets-with-scroll .shop-filters .sidebar-widget:not(.sidebar-slider):not(.etheme_widget_satick_block) > ul, .archive.woocommerce-page.s_widgets-with-scroll .sidebar .sidebar-widget:not(.sidebar-slider):not(.etheme_widget_satick_block) > div, .archive.woocommerce-page.s_widgets-with-scroll .shop-filters .sidebar-widget:not(.sidebar-slider):not(.etheme_widget_satick_block) > div {
                max-height:' . esc_html( $s_ws_height ) .'px;
            }';
        }
        ...
        $css .= '.page-heading {';
            if( ! empty( $bread_bg['background-color'] ) ){
                $css .= 'background-color:' . $bread_bg['background-color'] . ';';
            }
            ...
            if( ! empty( $bread_bg['background-attachment'] ) ){
                $css .= 'background-attachment:' . $bread_bg['background-attachment'] . ';';
            }

            if( ! empty( $bread_bg['background-size'] ) ){
                $css .= 'background-size:' . $bread_bg['background-size'] . ';';
            }

            if( ! empty( $bread_bg['background-repeat'] ) ){
                $css .= 'background-repeat:' . $bread_bg['background-repeat'] . ';';
            }

            if( ! empty( $bread_bg['background-position'] ) ){
                $css .= 'background-position:' . $bread_bg['background-position'] . ';';
            }
...
if( ! function_exists( 'etheme_add_opengraph_doctype' ) ) {
    function etheme_add_opengraph_doctype( $output ) {
        ...
        if ( is_array($share_facebook) && array_key_exists( 'share_facebook', $share_facebook['enabled'] ) ) {
...
if( ! function_exists('etheme_faceboook_login_button') ) {
    ...
    function etheme_faceboook_login_button($echo = true) {
        ...
        $return = '<div class="et-facebook-login-wrapper"><a href="' . esc_url( $facebook_login_url ) . '" class="et-facebook-login-button"><i class="et-icon et-facebook"></i> ' . esc_html__('Login / Register with Facebook', 'xstore') . '</a></div>';
        if ( $echo ) {
            echo $return;
        } else {
            return $return;
...
if ( ! function_exists( 'etheme_support_date' ) ) {
    function etheme_support_date(){
        ...
        if ( ! isset( $data['item'] ) || empty( $data['item'] ) || empty( $data['item']['supported_until'] ) ) return false;
...
if( ! function_exists('etheme_get_image') ) {
    function etheme_get_image($attach_id, $size, $location = '') {
        ...
        $class = ( $location == 'slider' ) ? 'swiper-lazy' : '';
...
if ( ! function_exists( 'et_activation_redirect' ) ) :
    function et_activation_redirect() {
        if ( class_exists( 'Redux' ) && etheme_is_activated() && ! isset( $_GET['page'] ) ) {
