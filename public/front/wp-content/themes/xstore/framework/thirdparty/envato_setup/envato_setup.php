...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function setup_wizard() {
			...
			wp_enqueue_style( 'etheme-admin-css', ETHEME_CODE_CSS . 'admin.css');

			if ( is_rtl() ) {
		    	wp_enqueue_style('etheme_admin_rtl_css', ETHEME_CODE_CSS.'admin-rtl.css', array('etheme-admin-css') );
		    }
		...
		public function envato_setup_default_plugins() {
			...
			<form method="post" class="plugins-form" data-version="<?php echo esc_attr( $version_import ); ?>">
	            ...
	            <div class="loading-info">
	                ...
	                <h2><?php esc_html_e('Please wait, it may take up to 2 minutes.', 'xstore'); ?></h2>
	                <div class="et-loader">
	                    ...
	                    <?php $i=0; foreach ($demos as $key => $version): $i++; ?>
	                        <div class="version-preview <?php if( $i==1 ) echo 'active-version '; echo ( in_array( $key, $versions_imported ) ) ? 'version-imported' : 'not-imported'; ?> version-preview-<?php echo esc_attr( $key ); ?>" data-version="<?php echo esc_attr( $key ); ?>">
	                            <div class="version-screenshot">
	                                <img src="https://xstore.8theme.com/dashboard-images/<?php echo esc_attr($key) ?>/screenshot.jpg" alt="<?php echo esc_attr($key) ?>">
	                                <a href="<?php echo esc_url( $version['preview_url'] ); ?>" target="_blank" class="button-preview">
	                                    <?php esc_html_e('Live preview', 'xstore'); ?>
	                                </a>
	                                <span class="et-button button-import-version no-loader" data-version="<?php echo esc_attr( $key ); ?>">
	                                    <?php echo ( ! in_array( $key, $versions_imported ) ) ? esc_html__('Choose', 'xstore') : esc_html__('Activate', 'xstore') ; ?>
	                                </span>
			...
			<ul class="support-blocks">
				<li>
					<a href="http://8theme.com/demo/xstore/previews/" target="_blank">
						<img src="<?php echo ETHEME_BASE_URI . ETHEME_CODE .'assets/images/'; ?>chat-icon.png">
						<span><?php esc_html_e( 'Live Chat 24/7', 'xstore' ) ?></span>
					</a>
				</li>
				<li>
	    ...
	    public function process_form() {
	        if( isset( $_POST['xstore-purchase-code'] ) && ! empty( $_POST['xstore-purchase-code'] ) ) {
	            ...
	            if( $response_code != '200' ) {
	                echo  '<p class="error et-message et-error">API request call error. Contact your server providers and ask to update OpenSSL system library to the 1.0 version.</p>';
	                return;
	            }
