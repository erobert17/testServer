...
if ( ! function_exists( 'etheme_menu_shortcode' ) ) {
	function etheme_menu_shortcode($atts) {
		...
		if ( $style == 'horizontal') {
			$class .= (!empty($submenu_side) ) ? ' submenu-'.$submenu_side : '';
		}
		else {
			$class .= (!empty($submenu_side_vertical) ) ? ' submenu-'.$submenu_side_vertical : '';
		}
		...
		$params = array(
			...
			array(
				...
				"value" => array(
					...
					esc_html__( "Simple list", 'xstore' ) => "menu-list",
				)
			),
			array(
				"type" => "dropdown",
			...
			array(
				"type" => "dropdown",
				"heading" => esc_html__( "Submenu side", 'xstore' ),
