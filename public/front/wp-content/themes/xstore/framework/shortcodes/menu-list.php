...
// ! Menu list 
// **********************************************************************// 
if ( !function_exists('etheme_menu_list_shortcode') ) {
  function etheme_menu_list_shortcode($atts, $content) {
    ...
    if( is_array($atts) && empty( $atts['title_google_fonts'] ) ) {
      ...
      $output .= '<ul class="et-menu-list ' . esc_attr( $class ) . ' text-' . $align . '">';

        $output .= '<li class="menu-item menu-item-type-post_type menu-item-object-page menu-item-has-children menu-parent-item menu-item-'.$li_id.'">';

        if( ! empty( $title ) ) {

          $output .= '<div class="item-title-holder '.$holder_class.'">';
          ...
          $output .= '<a href="'. $a_href . '" class="menu-title et-column-title '.$transform.'" title="'.$a_title. '" target="'. $a_target. '">'; 
        ...
        if ( !empty($content) ) {
          $output .= '<div class="menu-sublist"><ul>' . do_shortcode($content) . '</ul></div>';
        }
        $output .= '</li>';
      $output .= '</ul>';
      ...
      if ( !empty($hover_color)) {
        $output .= ' .menu-item-'.$li_id .' .item-title-holder:hover h2, .menu-item-'.$li_id .' .item-title-holder:active h2, ' . ' .menu-item-'.$li_id .'.current-menu-item .item-title-holder h2{ color:';
        $output .= $hover_color;
        $output .= ' !important;}';
      }
      if ( !empty($padding_top) || !empty($padding_right) || !empty($padding_bottom) || !empty($padding_left)) {
          $output .= '.menu-item-'.$li_id .' .item-title-holder  .menu-title {';
    ...
    //  if ( !empty($hover_bg)) {
     //    $output .= ' .menu-item-'.$li_id .':hover,' . ' .menu-item-'.$li_id .'.current-menu-item { background-color:';
     //    $output .= $hover_bg;
     //    $output .= ' !important;}';
    // }
      if ( !empty($spacing)) {
        $output .= ' .menu-item-'.$li_id .' > .item-title-holder h2 { letter-spacing:';
        $output .= $spacing;
        $output .= ';}';
      }
        $output .= ' .menu-item-'.$li_id .'.menu-parent-item > .item-title-holder .et-icon { ';
        $output .= $icon_style;
        $output .= '}';
        $output .= ' .menu-item-'.$li_id .'.menu-parent-item > .item-title-holder:hover .et-icon { ';
        $output .= $icon_style_hover;
        $output .= '}';
        $output .= '</style>';  
        
        // echo $this->singleParamHtmlHolder('this');
...
// **********************************************************************// 
// ! Register New Element: Menu list
// **********************************************************************//
add_action( 'init', 'etheme_register_menu_list');
if(!function_exists('etheme_register_menu_list')) {
  function etheme_register_menu_list() {
