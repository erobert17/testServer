...
if ( !function_exists('etheme_icon_box_shortcode') ) {
	function etheme_icon_box_shortcode($atts, $content) {
		...
		if($bg_color_hover != '' && $transparent_bg == '' ) {
			$style_hover .= 'background-color:' . $bg_color_hover . ';';
		}

		if ( $transparent_bg ) {
			$style .= 'background-color: transparent;';
			$style_hover .= 'background-color: transparent;';
		}
	...
	function etheme_register_vc_icon_box() {
	    ...
	    $params = array(
	      ...
	      'params' => array(
			...
			array(
				'type' => 'checkbox',
				'heading' => esc_html__( 'Transparent background color', 'xstore' ),
				'param_name' => 'transparent_bg',
				'description' => esc_html__( 'Icon without background color.', 'xstore' ),
			),
	        array(
	          "type" => "textfield",
