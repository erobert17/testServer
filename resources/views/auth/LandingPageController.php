...
class LandingPageController extends Controller
{
    ...
    public function link2($code){
        ...
        //check if owner is subscribed    
        if($authorUser->onPlan('monthlyPremium') == true){// author is subscribed, and current
          DB::table('landingPages')->where('user_id', $thisUser->id)->where('type', 'Property Details')->increment('views');
    ...
    public function openHouseLink($userId, $propertyId){
        ...
        DB::table("openHouseLeads")->insert(['propertyId' => $propertyId, 'hitType' => 'visit']);
        ...
        if($authorUser->onPlan('monthlyPremium') == true){// author is subscribed, and current
          ...
          return view('landingPageOpenHouse', compact('userId', 'thisProperty', 'author', 'authorAvatar', 'properties', 'recentImages', 'propertyId'));
        }
        ...
        //Must be the author, send a vaiable to be used as a warrning title.
          ...
          return view('landingPageOpenHouse', compact('userId', 'thisProperty', 'warningTitle', 'author', 'authorAvatar', 'properties', 'recentImages', 'propertyId'));
    }
    ...
    public function link4($code){
        ...
        DB::table("landingPages")->where('user_id', '=', $userId)->where('type', '=', 'New Product Countdown')->increment('views');
    ...
    public function link5($code){
        ...
        // record page visit !!!!!!
        DB::table("landingPages")->where('user_id', '=', $userId)->where('type', '=', 'New Product Coupon')->increment('views');
    ...
    public function link6($code){
        ...
        // record page visit !!!!!!
        DB::table("landingPages")->where('user_id', '=', $userId)->where('type', '=', 'Single Item Shopping Cart')->increment('views');
    ...
    public function editLandingPage6(){
        ...
        if(Auth::user()->subscribed('main') == false){
          ...
          $warningTitle = "You haven't <a href='/subscribe'>subscribed</a> yet. This landing page's link will only work for others once you subscribe.";
          //var_dump($warningTitle);
          return view('pages.landingSingleItemCartEdit', compact('landingPage', 'userLink', 'user', 'warningTitle', 'landingPageNumber', 'shippingPlans', 'product', 'shippingPlans', 'productImages'));
        }
        return view('pages.landingSingleItemCartEdit', compact('landingPage', 'userLink', 'user', 'landingPageNumber', 'shippingPlans', 'product', 'shippingPlans', 'productImages'));
    }
    ...
    public function allLandingPages()
    {
        $user = Auth::user();
        $userId = $user->id;
        $landingPages = DB::table('landingPages')->where('user_id', $user->id)->get();
        
        $userIndos = DB::table('userAssignedIndustries')->where(['userId' => $user->id, ])->get();
        ...
        for ($i=0; $i < count($landingPages); $i++) {
          ...
          if( in_array(1, $userIndos) == false ){
            //
            if($landingPage->type == '1' || $landingPage->type == '2' || $landingPage->type == '3'){
              
              unset($landingPages->{$i})
            }
        ...
        if(Auth::user()->subscribed('main') == false){
          ...
          $warningTitle = "You haven't <a href='/subscribe'>subscribed</a> yet. This landing page's link will only work for others once you subscribe.";
          //var_dump($warningTitle);
          return view('pages.allLandingPages', compact('landingPages', 'userLink', 'user', 'warningTitle'));
        }
        return view('pages.allLandingPages', compact('landingPages', 'userLink', 'user'));
    ...
    public function saveLandingPage6(Request $request){// coupon
          $user = Auth::user();
    ...
    public function orders(Request $request){
      ...
      $ordersActive = DB::table('orders')->where(['userId' => $user->id, 'enabled' => 1, 'archived' => 0])->get();
