...
class ProfilesController extends Controller
{
    ...
    public function dashboard(Request $request){
        ...
        $storedLastLeads = DB::table('orders')->where(['user_id' => $user->id])->count();
        if( count($storedLeads) > 0){
          $storedLeads = $storedLeads[0];
        }else{// if row doesn't exist in leadsLastLogin, add this lp type to row
         DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Shopping Cart'])->insert(['userId' =>$user->id, 'leads' => $storedLastLeads, 'difference' => $difference5, 'type' => 'Shopping Cart']);
        }
        if($storedLeads < $storedLastLeads){//Leads have changed
          $difference5 = ($storedLeads - $storedLastLeads) * -1;
          DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Shopping Cart'])->update(['leads' => $storedLastLeads, 'difference' => $difference5]);
        }
        ...
        return view('pages.user.dashboard', compact('hintText' ,'userHintState','user', 'countPages', 'totalViews', 'isRealestate', 'orders', 'timeStamps1', 'leads','graphData', 'difference1', 'difference2', 'difference3'));
